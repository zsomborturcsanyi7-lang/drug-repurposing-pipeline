...
def step_screen(pdb_id: str, config: dict, depth: str = 'standard'):
    ...
    # Run AI screening
    if os.path.exists("ai_smart_search.py"):
        cmd = [sys.executable, "ai_smart_search.py"]
        result = subprocess.run(cmd, capture_output=True, text=True)
...
def step_screen_similarity(pdb_id: str, config: dict, max_ligands: int):
    ...
    if os.path.exists("production_screening.py"):
        cmd = [sys.executable, "production_screening.py"]
...
def step_prepare_ligands(hits_csv: str, config: dict):
    ...
    cmd = [sys.executable, "prep_ligands.py", hits_csv]
    result = subprocess.run(cmd, capture_output=True, text=True)
    logger.info(result.stdout.strip())
    if result.stderr:
...
def step_dock(pdb_id: str, config: dict):
    ...
    if os.path.exists("run_parallel_docking.py"):
        cmd = [sys.executable, "run_parallel_docking.py"]
    elif os.path.exists("run_docking_parallel.py"):
        cmd = [sys.executable, "run_docking_parallel.py"]
    else:
...
def step_parse_results(pdb_id: str):
    ...
    if os.path.exists("parse_docking_results.py"):
        cmd = [sys.executable, "parse_docking_results.py"]
        result = subprocess.run(cmd, capture_output=True, text=True)
        logger.info(result.stdout.strip())
...
def step_optimize(pdb_id: str, config: dict):
    ...
    if os.path.exists("optimize_lead.py"):
        cmd = [sys.executable, "optimize_lead.py", "--pdb", pdb_id]
        subprocess.run(cmd)
    elif os.path.exists("mutate_lead.py"):
        cmd = [sys.executable, "mutate_lead.py"]
        subprocess.run(cmd)
    else:
